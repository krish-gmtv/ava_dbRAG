...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Embed buyer-quarter docs with Ava in batches and "
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="Maximum number of docs to embed and load.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=100,
        help="Number of docs per embeddings batch request to Ava.",
    )
    parser.add_argument(
        "--pg-host",
        type=str,
    ...
    try:
        ...
        if not filtered_docs:
            logger.warning(
                "All loaded docs had empty embedding_text; nothing to embed/load."
            )
            return
        ...
        logger.info(
            "Embedding and loading %d docs in batches of %d...",
            total,
            batch_size,
        )
        ...
        for start in range(0, total, batch_size):
            ...
            embeddings = fetch_embeddings(batch_texts, token)
            logger.info("Received %d embeddings for current batch.", len(embeddings))

            if len(embeddings) != len(batch_docs):
                raise RuntimeError(
                    f"Embedding count mismatch: got {len(embeddings)} for {len(batch_docs)} batch docs"
                )
            for i, emb in enumerate(embeddings):
                if len(emb) != EMBEDDING_DIMENSION:
                    raise RuntimeError(
                        f"Embedding at batch index {i} has dimension {len(emb)} instead of {EMBEDDING_DIMENSION}"
                    )

            try:
                upsert_vectors(conn, batch_docs, embeddings)
            except Exception:
                conn.rollback()
                logger.exception(
                    "Failed while upserting batch %d/%d",
                    (start // batch_size) + 1,
                    total_batches,
                )
                raise

            # Gentle pause between batches to avoid overloading the embeddings API
            if end < total:
                time.sleep(2.0)
    finally:
        conn.close()
    ...
    logger.info(
        "Successfully upserted %d docs into buyer_quarter_vectors.",
        len(filtered_docs),
    )
