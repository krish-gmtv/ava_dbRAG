...
This shim must work when running from repo root (tests) and when running
``python scripts/<something>.py`` (sys.path contains ``scripts/``).
"""
...
_impl_path = Path(__file__).resolve().parent / "reporting" / "report_normalizer_v2.py"
